...
(define-module (nezzar import go)
...
build a package."
  ...
  (define (module-path->module-meta module-path)
    (let ((target-vcs (find (lambda (vcs)
			      (string-prefix? (vcs-url-prefix vcs) module-path))
			    known-vcs)))
      (make-module-meta (module-path->repository-root module-path)
			(vcs-type target-vcs)
			(string-append "https://" module-path))))
  ...
  (let* ((meta-data (http-fetch* (format #f "https://~a?go-get=1" module-path)))
	...
	(or
     ...
     ;; Fallback to known vcs definition if import prefix not present.
     (module-path->module-meta module-path))))
...
(define* (vcs->origin vcs-type vcs-repo-url version #:key (tag-prefix ""))
          ...
          (sha256
           (base32
            ,(bytevector->nix-base32-string
              (git-checkout-hash vcs-repo-url (go-version->git-ref
					       (string-append tag-prefix
							      version))
...
When VERSION is unspecified, the latest version available is used."
  (let* ((available-versions (go-module-available-versions goproxy module-path))
	 ...
	 (vcs-repo-subdir
	  (match:suffix
	   (or (string-match (regexp-quote (module-meta-import-prefix meta-data))
			     module-path)
	       (string-match (regexp-quote root-module-path)
			     module-path))))
	 (vcs-repo-vers-match (string-match "/v([0-9]|\\.)*$" vcs-repo-subdir))
	 (vcs-repo-tag-prefix (string-trim
			       (string-append
				(if vcs-repo-vers-match
				    (match:prefix vcs-repo-vers-match)
				    vcs-repo-subdir)
				"/")
			       #\/))
	 ;; (not (string-match "^v[0-9]" vcs-repo-subdir))
     ...
     `(package
        ...
        (source
         ,(vcs->origin vcs-type vcs-repo-url version*
		       #:tag-prefix vcs-repo-tag-prefix))
...
(pretty-print (go-module-recursive-import "github.com/apptainer/apptainer"))
